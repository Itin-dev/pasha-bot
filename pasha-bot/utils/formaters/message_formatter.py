...
def format_messages(messages):
    ...
    try:
        ...
        for thread_id, username, date, message_content in messages:
            ...
            grouped_messages[thread_id].append((username, message_content))  # Removed date here
        ...
        for thread_id, msgs in sorted_threads:
            ...
            for username, message_content in reversed(msgs):  # Reverse to get chronological order
                logging.info(f"Formatting message: {username}, {message_content}")
                formatted_messages.append(f"  - {username}: {message_content}")  # Removed date here
